...
pub fn encrypt_xchacha(key: &[u8;32], plaintext: &[u8]) -> (Vec<u8>, [u8;24]) {
    ...
    let ct = cipher.encrypt(nonce, Payload { msg: plaintext, aad: b"DOC" }).expect("encryption failure");
    (ct, nonce_bytes)
}

pub fn decrypt_xchacha(key: &[u8;32], ciphertext: &[u8], nonce_bytes: &[u8;24]) -> Option<Vec<u8>> {
    let cipher = XChaCha20Poly1305::new(key.into());
    let nonce = XNonce::from_slice(nonce_bytes);
    cipher.decrypt(nonce, Payload { msg: ciphertext, aad: b"DOC" }).ok()
}
